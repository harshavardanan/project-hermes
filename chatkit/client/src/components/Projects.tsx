    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white/[0.02] rounded-2xl h-[280px] p-6 flex flex-col">
  ...
  if (error) {
    return (
      <div className="p-10 text-center bg-white/[0.02] rounded-2xl max-w-md mx-auto mt-10">
        ...
        <p className="text-slate-400 text-sm mb-6">{error instanceof Error ? error.message : String(error)}</p>
        <Button
          variant="outline"
          ...
          >
            ...
            <div className="relative flex flex-col items-center p-8 md:p-16 text-center w-full h-full justify-center">
              <div className="relative w-20 h-20 mb-8 flex items-center justify-center">
                <div className="absolute inset-0 bg-brand-primary/10 rounded-2xl rotate-6 group-hover:rotate-12 group-hover:scale-110 transition-all duration-500" />
                <div className="absolute inset-0 bg-[#111]/80 backdrop-blur-md rounded-2xl -rotate-3 group-hover:-rotate-6 group-hover:scale-110 transition-all duration-500 flex items-center justify-center shadow-xl">
                  <Globe
                    className="text-white/40 group-hover:text-brand-primary transition-colors duration-500"
                    size={32}
                    strokeWidth={1.5}
                  />
                </div>
          ...
          projects.map((p) => (
            ...
            >
              <div className="bg-white/[0.02] hover:bg-white/[0.04] rounded-2xl transition-all duration-300 h-full flex flex-col overflow-hidden relative border-none">
                ...
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-white/5 group-hover:bg-brand-primary transition-colors duration-500" />

                {/* Soft glow that follows the hover state */}
                <div className="absolute -inset-1 bg-gradient-to-r from-brand-primary/0 via-brand-primary/5 to-transparent opacity-0 group-hover:opacity-100 blur-xl transition-opacity duration-500 pointer-events-none" />

                <div className="p-6 relative z-10 flex flex-col h-full">
                  <div className="flex flex-row items-start justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div className="p-3 rounded-xl bg-black/40 text-white/50 group-hover:bg-brand-primary/10 group-hover:text-brand-primary transition-colors duration-300">
                        <Terminal size={20} strokeWidth={1.5} />
                      </div>
                      <h3 className="text-xl font-bold text-white tracking-tight group-hover:text-brand-primary transition-colors">
                        {p.projectName}
                      </h3>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-white/[0.05] flex items-center justify-center group-hover:bg-brand-primary group-hover:text-brand-primary-fg text-white/30 transition-all duration-300">
                      <ChevronRight
                        size={16}
                        strokeWidth={2}

...
export const sendMessage = async (input) => {
    ...
    if (project && project.plan) {
        ...
        if (usedToday >= dailyLimit) {
            console.log(`[Hermes] 🛑 LIMIT HIT: Project ${project.projectName} (${usedToday}/${dailyLimit})`);
    ...
    if (project?._id) {
        try {
            ...
            console.log(`[Hermes Engine] 🪙 Logged ${tokensUsed} tokens for ${project.projectName}`);
        }
        catch (err) {
...
export const getHistory = async (roomId, hermesUserId, before, limit = PAGE_SIZE) => {
    ...
    const query = { roomId: new Types.ObjectId(roomId), isDeleted: false };
    if (before) {
...
// ── Mark seen ─────────────────────────────────────────────────────────────────
export const markSeen = async (roomId, hermesUserId, lastMessageId) => {
...
// ── Add/toggle reaction ───────────────────────────────────────────────────────
export const addReaction = async (messageId, hermesUserId, emoji) => {
    const message = await Message.findById(messageId);
    if (!message || message.isDeleted)
        return { error: "Message not found" };
    const existing = message.reactions.find((r) => r.emoji === emoji);
    if (existing) {
        const hasReacted = existing.users.some((u) => u.toString() === hermesUserId);
        if (hasReacted) {
            existing.users = existing.users.filter((u) => u.toString() !== hermesUserId);
...
// ── Soft delete message ───────────────────────────────────────────────────────
export const deleteMessage = async (messageId, hermesUserId) => {
    const message = await Message.findById(messageId);
    if (!message)
...
// ── Edit message ──────────────────────────────────────────────────────────────
export const editMessage = async (messageId, hermesUserId, newText) => {
    const message = await Message.findById(messageId);
    if (!message || message.isDeleted)
        return { error: "Message not found" };
    if (message.senderId.toString() !== hermesUserId)
    ...
    if (message.type !== "text")
        return { error: "Can only edit text messages" };

...
import { hermesAuth, validateProjectCredentials, signHermesToken, } from "./middleware/auth.js";
...
export const initHermes = (io, app) => {
    // ── Cloudinary ──────────────────────────────────────────────────────────────
    cloudinary.config({
    ...
    hermesRouter.post("/connect", async (req, res) => {
        try {
            ...
            const { valid, project, error } = await validateProjectCredentials(apiKey, secret);
            if (!valid)
            ...
            // With this:
            const hermesUser = await HermesUser.findOneAndUpdate({ externalId: userId, projectId: project._id }, {
                $set: {
                    displayName,
                    ...(avatar && { avatar }),
                    ...(email && { email }),
            ...
            logger.info(`Hermes connect: ${displayName} (${hermesUser._id}) via "${project.projectName}"`);
            res.json({
                success: true,
                token,
                user: {
    ...
    // ── GET /hermes/users ─────────────────────────────────────────────────────
    hermesRouter.get("/users", hermesAuth, async (req, res) => {
        try {
            const { projectId } = req.hermesUser;
